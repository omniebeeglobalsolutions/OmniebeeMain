    ...
    <section className="py-12 px-4 sm:px-6 lg:px-20 bg-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
        {visibleTestimonials.map((client, idx) => (
          ...
          >
            <div className="bg-gradient-to-r from-[#2E3E95]/90 to-[#1E232A]/90 p-5 sm:p-6 text-white flex flex-col justify-between h-full">
              <p className="text-[14px] leading-relaxed font-inter mb-4 whitespace-pre-line">
        ...
        {Array.from({ length: totalPages }, (_, index) => (
          <button
            ...
            className={`px-4 py-1 rounded-full border text-sm font-medium ${
              currentPage === index
                ? "bg-[#2E3E95] text-white"
                : "bg-white border-gray-300 text-[#2E3E95]"
            } hover:bg-[#2E3E95]/90 hover:text-white transition`}
          >
            {index + 1}
          </button>
        ))}
